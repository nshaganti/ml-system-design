...
heuristic), Phase 1's two-tower **beats** the heuristic here by +84% recall on the
...
| **Two-tower retrieval (ID + side features)** | Scales retrieval; strong when feedback is dense. | **BUILT (Phase 1), wins +84%; side features are the next step** |
...
+61% session lift, a +84% retrieval win, and a sobering +100% evaluation bias.
